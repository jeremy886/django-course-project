...
class CartPageTest(SplinterTestCase):
    ...
    def test_user_cart_with_items(self):
        ...
        quantity_fields = self.browser.find_by_css("input[name=quantity]")
        self.assertEqual([e.value for e in quantity_fields], ['9'])
        ...
        quantity_fields = self.browser.find_by_css("input[name=quantity]")
        self.assertEqual([e.value for e in quantity_fields], ['9', '7'])
